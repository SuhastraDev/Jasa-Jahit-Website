...
<div class="max-w-2xl mx-auto px-4 sm:px-6 py-8">
    ...
    <div class="flex items-center gap-4 mb-8">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Edit Katalog Desain</h1>
            <p class="text-gray-500 text-sm mt-0.5">Perbarui informasi katalog <span class="font-semibold text-gray-700">{{ $catalog->name }}</span></p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-100 bg-gray-50/60">
            <div class="flex items-center gap-3">
                ...
                <span class="font-semibold text-gray-700 text-sm">Ubah Detail Katalog</span>
            </div>
        </div>

        <form action="{{ route('admin.catalogs.update', $catalog) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-5">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select id="service_id" name="service_id" required
                    ...
                    @foreach($services as $service)
                        <option value="{{ $service->id }}" {{ old('service_id', $catalog->service_id) == $service->id ? 'selected' : '' }}>
                            {{ $service->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="text" id="name" name="name" value="{{ old('name', $catalog->name) }}" required
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="3" required
                    class="w-full px-4 py-3 border border-gray-200 rounded-xl text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all text-sm resize-none @error('description') border-red-400 @enderror">{{ old('description', $catalog->description) }}</textarea>
                @error('description')
                    <p class="mt-1.5 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            {{-- Gambar --}}
            <div x-data="{ preview: '{{ $catalog->image_path ? Storage::url($catalog->image_path) : '' }}', fileName: '' }">
                <label class="block text-sm font-semibold text-gray-700 mb-1.5">Gambar Referensi</label>
            ...
            <div>
                ...
                <div class="flex gap-3">
                    <label class="flex-1 flex items-center gap-3 border border-gray-200 rounded-xl px-4 py-3 cursor-pointer hover:border-blue-300 hover:bg-blue-50/30 transition-all has-[:checked]:border-blue-500 has-[:checked]:bg-blue-50">
                        <input type="radio" name="is_active" value="1" {{ old('is_active', $catalog->is_active) == '1' ? 'checked' : '' }} class="text-blue-600 focus:ring-blue-500">
                        <div>
                    ...
                    <label class="flex-1 flex items-center gap-3 border border-gray-200 rounded-xl px-4 py-3 cursor-pointer hover:border-gray-300 hover:bg-gray-50 transition-all has-[:checked]:border-gray-400 has-[:checked]:bg-gray-50">
                        <input type="radio" name="is_active" value="0" {{ old('is_active', $catalog->is_active) == '0' ? 'checked' : '' }} class="text-gray-600 focus:ring-gray-500">
                        <div>
