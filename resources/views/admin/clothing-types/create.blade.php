...
@section('page-title', 'Tambah Jenis Pakaian')
@section('content')
<div class="max-w-2xl mx-auto px-4 sm:px-6 py-8">
    ...
    <div class="flex items-center gap-4 mb-8">
        ...
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Tambah Jenis Pakaian</h1>
            <p class="text-gray-500 text-sm mt-0.5">Jenis pakaian baru akan muncul sebagai pilihan di form pemesanan pelanggan</p>
        </div>
    </div>

    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <form action="{{ route('admin.clothing-types.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-5">
            @csrf
            ...
            <div>
                ...
                <input type="text" id="name" name="name" value="{{ old('name') }}" required
                    placeholder="Contoh: Kemeja, Gamis, Baju Koko..."
            ...
            <div>
                ...
                <div class="grid grid-cols-3 gap-3">
                    ...
                    <label class="flex items-center justify-center gap-2 border border-gray-200 rounded-xl px-3 py-3 cursor-pointer hover:border-blue-300 hover:bg-blue-50/30 transition-all has-[:checked]:border-blue-500 has-[:checked]:bg-blue-50">
                        <input type="radio" name="gender" value="{{ $value }}" {{ old('gender', 'unisex') === $value ? 'checked' : '' }} class="text-blue-600 focus:ring-blue-500">
                        <span class="text-sm font-semibold text-gray-800">{{ $label }}</span>
                    </label>
                    @endforeach
                </div>
                <p class="mt-1.5 text-xs text-gray-400">Menentukan jenis pakaian ini akan tampil untuk gender apa saat pelanggan checkout. Pilih Unisex kalau cocok untuk semua.</p>
                @error('gender')
                    <p class="mt-1.5 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <div class="grid grid-cols-3 gap-3">
                    ...
                    <label class="flex items-center justify-center gap-2 border border-gray-200 rounded-xl px-3 py-3 cursor-pointer hover:border-blue-300 hover:bg-blue-50/30 transition-all has-[:checked]:border-blue-500 has-[:checked]:bg-blue-50">
                        <input type="radio" name="category" value="{{ $value }}" {{ old('category', 'baju') === $value ? 'checked' : '' }} class="text-blue-600 focus:ring-blue-500">
                        <span class="text-sm font-semibold text-gray-800">{{ $label }}</span>
                    </label>
                    @endforeach
                </div>
            ...
            <div x-data="{ preview: null, fileName: '' }">
                <label class="block text-sm font-semibold text-gray-700 mb-1.5">
                    Foto Referensi <span class="text-gray-400 font-normal text-xs">(opsional)</span>
                </label>
                <div class="border-2 border-dashed border-gray-200 rounded-xl p-6 text-center hover:border-blue-400 transition-colors cursor-pointer"
                    ...
                    <template x-if="!preview">
                        <div class="space-y-3">
                            ...
                            <div>
                                <p class="text-sm font-medium text-gray-700">Klik untuk upload foto contoh jenis pakaian ini</p>
                                <p class="text-xs text-gray-400 mt-1">PNG, JPG — Maks. 10MB</p>
                            </div>
                        </div>
                    </template>
