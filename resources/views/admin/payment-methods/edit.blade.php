...
<div class="max-w-2xl mx-auto px-4 sm:px-6 py-8">
    ...
    <div class="flex items-center gap-4 mb-8">
        ...
        <div>
            ...
            <p class="text-gray-500 text-sm mt-0.5">{{ $paymentMethod->name }}</p>
        </div>
    </div>

    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <form action="{{ route('admin.payment-methods.update', $paymentMethod) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-5">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" id="name" name="name" value="{{ old('name', $paymentMethod->name) }}" required
                    class="w-full px-4 py-3 border border-gray-200 rounded-xl text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all text-sm @error('name') border-red-400 @enderror">
                @error('name')
            ...
            <div>
                ...
                <input type="text" id="account_number" name="account_number" value="{{ old('account_number', $paymentMethod->account_number) }}" required
                    class="w-full px-4 py-3 border border-gray-200 rounded-xl text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all text-sm @error('account_number') border-red-400 @enderror">
            ...
            <div>
                ...
                <input type="text" id="account_name" name="account_name" value="{{ old('account_name', $paymentMethod->account_name) }}" required
                    class="w-full px-4 py-3 border border-gray-200 rounded-xl text-gray-900 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all text-sm @error('account_name') border-red-400 @enderror">
                @error('account_name')
                    <p class="mt-1.5 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </div>

            <div x-data="{ preview: '{{ $paymentMethod->qr_image ? Storage::url($paymentMethod->qr_image) : '' }}', fileName: '' }">
                <label class="block text-sm font-semibold text-gray-700 mb-1.5">QR Code</label>
            ...
            <div>
                ...
                <textarea id="instructions" name="instructions" rows="4"
                    placeholder="Kosongkan untuk memakai petunjuk umum."
                    class="w-full px-4 py-3 border border-gray-200 rounded-xl text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all text-sm resize-none @error('instructions') border-red-400 @enderror">{{ old('instructions', $paymentMethod->instructions) }}</textarea>
                @error('instructions')
                    <p class="mt-1.5 text-xs text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <div class="flex gap-3">
                    <label class="flex-1 flex items-center gap-3 border border-gray-200 rounded-xl px-4 py-3 cursor-pointer hover:border-blue-300 hover:bg-blue-50/30 transition-all has-[:checked]:border-blue-500 has-[:checked]:bg-blue-50">
                        <input type="radio" name="is_active" value="1" {{ old('is_active', $paymentMethod->is_active ? '1' : '0') == '1' ? 'checked' : '' }} class="text-blue-600 focus:ring-blue-500">
                        <div>
                    ...
                    <label class="flex-1 flex items-center gap-3 border border-gray-200 rounded-xl px-4 py-3 cursor-pointer hover:border-gray-300 hover:bg-gray-50 transition-all has-[:checked]:border-gray-400 has-[:checked]:bg-gray-50">
                        <input type="radio" name="is_active" value="0" {{ old('is_active', $paymentMethod->is_active ? '1' : '0') == '0' ? 'checked' : '' }} class="text-gray-600 focus:ring-gray-500">
                        <div>
